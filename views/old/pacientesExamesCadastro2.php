						...
						<form  id="admin"  name="admin" class="form-horizontal" action="../controllers/pacienteExamesCadastro.php?idP=<?php echo $idPaciente;?>" method="POST">		
                            ...
                            <?php
                                if($temExame == 1)
                                    {
                                        $html = "<hr>
                                                <h7 class='text-success'>Dados do Último Exame</h7>
                                                <div class='row'>
                                                    <div class='col px-md-1 col-md-2'>
                                                        <label for='inputSuccess' class='control-label'>data:</label>
                                                        <input type='text' class='form-control' value = '$datUltimoCad' readonly tabindex='-1'>
                                                    </div>
                    
                                                    <div class='col px-md-1 col-md-10'>
                                                        <label for='inputSuccess' class='control-label'>Exames Realizados:</label>
                                                        <input type='text'  class='form-control' 
                                                        <?php $tiposExames =  unserialize($tiposExames);
                                                                $t = sizeof($tiposExames);
                                                                $result = count($tiposExames);
                                                                for ($i = 0; $i < $t; $i++) 
                                                                    {
