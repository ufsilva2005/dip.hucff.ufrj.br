    ...
    class SistemaDAO 
        {
            ...
            public function ReimprimirEtiquetas($dataCadastro) 
                {					
                    $conn = Database::connect();    
                    $conn->exec('SET CHARACTER SET utf8');    
                    $sql = "SELECT * FROM examesSolicitados e, paciente p, origem o
                    ...
                    AND p.idPaciente = e.idPaciente
                    AND o.idOrigem = e.idOrigem
                    AND e.dataCadastroE = '$dataCadastro'
                    GROUP BY e.idExamesSolicitados";
            ...
            public function ImprimirEtiquetas($idFuncionario) 
                {					
                    $conn = Database::connect();    
                    $conn->exec('SET CHARACTER SET utf8');    
                    $sql = "SELECT * FROM examesSolicitados e, paciente p, origem o
                    WHERE e.idFuncionario = '$idFuncionario' 
                    AND e.imprimir = 0
                    AND p.idPaciente = e.idPaciente
                    AND o.idOrigem = e.idOrigem
                    GROUP BY e.idExamesSolicitados";
            ...
            public function ExamePacienteUltimo($idPaciente)
                {					
                    $conn = Database::connect();    
                    $conn->exec('SET CHARACTER SET utf8');    
                    $sql = "SELECT * FROM examesSolicitados
                    WHERE  idPaciente = '$idPaciente'
                    ORDER BY idExamesSolicitados DESC LIMIT 1";
            ...
            public function PesquisaExameRetificar($tipoPesquisa,$valor,$dataInicio,$dataFinal)
                {
                    $conn = Database::connect();                                
                    $conn->exec('SET CHARACTER SET utf8');                                
                    $sql = "SELECT * FROM examesSolicitados e, paciente p
                    WHERE $tipoPesquisa LIKE '$valor%'
                    AND p.idPaciente = e.idPaciente
                    AND e.dataAmostra BETWEEN '$dataInicio' AND '$dataFinal'
                    GROUP BY e.idExamesSolicitados";
            ...
            public function ListarExameRetificar($idPaciente,$idE)
                {
                    $conn = Database::connect();                                
                    $conn->exec('SET CHARACTER SET utf8');                                
                    $sql = "SELECT * FROM examesSolicitados e, paciente p,  origem o
                    WHERE e.idExamesSolicitados = '$idE'
            ...
            public function RetificarExamePaciente($idPaciente, $idExameAnt, $tiposExamesBd, $numOrigemBd) 
				{
					$conn = Database::connect();
					
					$conn->exec('SET CHARACTER SET utf8');
					
					$sql = "UPDATE examesSolicitados 
                    SET idOrigem = '$numOrigemBd',  
                    tiposExames = '$tiposExamesBd'
                    WHERE idPaciente = '$idPaciente'
					AND idExamesSolicitados = '$idExameAnt'";
					try 
						{
							$listar = $conn->query($sql);
							$buscar = $listar->fetchAll(PDO::FETCH_OBJ);
            ...
            public function PacienteCadastrar($paciente) 
                {
                    $conn = Database::connect();					
                    $conn->exec('SET CHARACTER SET utf8');
                    $prepara = $conn->prepare("INSERT INTO paciente (nomePaciente,  prontuario, gestante, numSus, cpf, sexo, dataNasce, identidade, 
                    orgaoEmissor, naturalidade, nacionalidade, dataCadastroP, idFuncionario, dataAltCad, idFuncionarioAlt)
                    VALUES(:BdnomePaciente, :Bdprontuario, :Bdgestante, :BdnumSus, :Bdcpf, :Bdsexo, :BddataNasce, :Bdidentidade, :BdorgaoEmissor, 
            ...
            public function PacienteUpdate($paciente) 
                {
                    ...
                    $prepara = $conn->prepare("UPDATE paciente SET  nomePaciente=:BdnomePaciente, prontuario=:Bdprontuario, 
                    gestante=:Bdgestante, numSus=:BdnumSus, cpf=:Bdcpf, sexo=:Bdsexo, dataNasce=:BddataNasce, identidade=:Bdidentidade, 
            ...
            //INÍCIO DAS FUNCOES RELACIONADAS AOS FUNCIONÁRIOS
